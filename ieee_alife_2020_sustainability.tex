...
$(\couplingconstant_{ji})$ is an upper triangular matrix. \dpnote{is that what you
  wanted?}
...
simple example, setting $\couplingconstant_{ij} = -\couplingconstant_{ji}$ \dpnote{is that for a
...
characterise empowerment. We allow the agent to impact the
environment by choosing to set $\agentimpact_i$ to $-\impactstrength$, $0$ or $\impactstrength$,
where $\impactstrength$ is a parameter setting the strength of the impact, or
control, of the agent on the environment. A setting of
...
e.g.\ if one had $\impactstrength \gg \ccrit + \max_i \couplingfunction_i$, the agent could directly
...
setting $\impactstrength = 0$ prevents the agent from having any impact on the
...
at the end of this relaxation period is defined to be $\impactstrength$-accessible
...
previously developed \cite{Kim2009_sustainability}. The cubic differential
...
\end{equation}
where $\delta_{ji}$ is Kronecker's delta. \dpnote{Check that this is
  not a repeat}
